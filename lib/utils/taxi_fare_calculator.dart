...
class TaxiFareCalculator {
  ...
  static int calculateFare(
    ...
    // تطبيق وقت الليل: سعر ثابت 1000 بعد الساعة 10:30 مساءً
    // وقت الليل له أولوية على وقت الذروة
    if (isNight) {
      baseFare = 1000; // سعر ثابت 1000 بعد الساعة 10:30 مساءً
    } else if (isPeakTime) {
  ...
  /// تحديد إذا كان وقت الليل (بعد 10:30 مساءً أو قبل 6 صباحاً)
  static bool isNightTime() {
    final now = DateTime.now();
    final hour = now.hour;
    final minute = now.minute;
    
    // بعد الساعة 10:30 مساءً (22:30) أو قبل 6 صباحاً
    if (hour < 6) {
      return true; // قبل 6 صباحاً
    }
    if (hour > 22 || (hour == 22 && minute >= 30)) {
      return true; // بعد 22:30
    }
    return false;
  }
}
